...
## 2026-05-22 - Conditional ARIA Controls
**Learning:** When using `aria-controls` to link a toggle button to a collapsible panel that is conditionally rendered based on complex logic, the `aria-controls` attribute must use the exact same condition to evaluate to `undefined` when the panel is not rendered, preventing it from referencing a non-existent ID.
**Action:** Always conditionally apply `aria-controls` using logic that strictly matches the target element's exact rendering condition when the target is conditionally rendered.
