...
## 2026-08-04 - Accessible Loading States
**Learning:** The Storage page had visual-only loading text ("Searching...") that wasn't properly announced by screen readers or visually engaging.
**Action:** Always implement a complete loading state pattern utilizing `Loader2` combined with visually hidden (`sr-only`) and `aria-hidden` attributes inside a `role="status"` container to ensure accessibility for all users while providing smooth visual feedback.
