...
## 2026-08-26 - Focus indicators for interactive grid items
**Learning:** Large interactive grid items (like the connection buttons) without explicit focus styling make it difficult for keyboard users to track their current position within the grid. This is particularly problematic for elements that are purely visual without traditional interactive form styles.
**Action:** Always ensure large interactive elements used in grids or lists include explicit focus styling using `focus:outline-none focus-visible:ring-2`.
