...
**Learning:** Toggle buttons with `aria-expanded` that control conditionally rendered React components (unmounted when closed) cause invalid ARIA references when closed because the target `id` doesn't exist.
**Action:** Always conditionally apply `aria-controls={isOpen ? 'panel-id' : undefined}` for target elements that are conditionally mounted/unmounted.
