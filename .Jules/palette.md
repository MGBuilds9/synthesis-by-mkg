...
## 2026-07-25 - Status Indicator Interactivity
**Learning:** Status indicators that represent configurable entities (like connected accounts) are frustrating when they are non-interactive elements (like `div`), forcing users to navigate to settings separately.
**Action:** Always wrap visual status indicators of configurable entities in a `Link` component directed to their settings page, and provide comprehensive screen-reader context combining both the entity name and its current status in the `aria-label`.
