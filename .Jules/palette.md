...
## 2026-08-02 - Interaction Feedback
**Learning:** Empty states without a clear visual indicator often leave users feeling lost or unsure of the next step. Simple, descriptive empty states with icons significantly improve user understanding and onboarding.
**Action:** Enhance empty states (e.g. storage files list when no files are synced) with a descriptive icon, a bold main title, and subtitle text guiding the user towards a resolution (such as adjusting filters or connecting an integration). Additionally, handle the loading state using the standard `Loader2` accessible visual feedback.
