...
## 2026-08-28 - Select Dropdown Accessibility
**Learning:** Visual labels placed next to `<select>` elements without using `htmlFor` and `id` do not provide accessible names, causing screen readers to announce unlabelled controls.
**Action:** Always include an explicit `aria-label` attribute on `<select>` elements when visual labels aren't strictly linked, ensuring proper context is announced by screen readers.
