...
## 2026-08-23 - Select Dropdown Accessibility
**Learning:** Visual labels next to `<select>` dropdowns do not automatically associate with the input without `htmlFor` and an `id`, leaving them unannounced by screen readers.
**Action:** Always include an explicit `aria-label` attribute on `<select>` elements when not using `htmlFor`/`id` linking, and update related React Testing Library tests to use `getByRole('combobox', { name: /.../i })` to validate the accessibility.
