...
## 2024-05-24 - Filter Toggle Accessibility\n**Learning:** Filter toggle buttons that use `aria-expanded` must include an `aria-controls` attribute pointing to the ID of the controlled panel. If the panel is conditionally rendered, the `aria-controls` attribute must conditionally point to the ID only when the panel is mounted (e.g. `aria-controls={showFilters ? 'filter-panel' : undefined}`) to prevent referencing non-existent elements.\n**Action:** Add conditional `aria-controls` to filter toggles and assign a matching `id` to the filter panel.
