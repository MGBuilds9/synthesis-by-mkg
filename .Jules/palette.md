...
## 2024-05-24 - Conditional ARIA Controls
**Learning:** When using `aria-controls` on a toggle button that controls a conditionally rendered target element, the attribute must conditionally match the exact rendering logic of the target to prevent referencing a non-existent ID.
**Action:** Apply conditional logic to `aria-controls` matching the target's rendering condition (e.g., `aria-controls={isOpen && hasOptions ? 'panel' : undefined}`) and ensure the target has the matching `id`.
