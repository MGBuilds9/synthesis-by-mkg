...
## 2026-08-21 - Select Dropdown Accessibility
**Learning:** When implementing visual labels next to `<select>` dropdowns without using `htmlFor` and an `id`, the `<select>` element lacks context for screen readers.
**Action:** Always include an explicit `aria-label` attribute on the `<select>` element to ensure proper context is announced by screen readers.
