...
## 2026-05-20 - Toggle Button Controls
**Learning:** Expandable panels (like filters) toggled by a button often use `aria-expanded` but lack `aria-controls`, which makes it hard for screen readers to associate the button with the panel it controls. When panels are conditionally rendered, naive static `aria-controls` bindings can reference non-existent elements, causing invalid ARIA references.
**Action:** When implementing interactive toggle buttons (e.g., drop-downs or filter panels) with `aria-expanded`, ensure the button also includes the `aria-controls` attribute pointing to the `id` of the controlled element. In React, if the target element is conditionally rendered, conditionally apply the attribute using logic that strictly matches the target element's exact rendering condition (e.g., `aria-controls={isOpen && hasOptions ? 'panel' : undefined}`) to prevent referencing a non-existent ID.
