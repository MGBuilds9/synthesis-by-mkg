...
## $(date +%Y-%m-%d) - Unassociated Select Labels
**Learning:** Visual labels placed next to `<select>` dropdowns without using `htmlFor` and an `id` leave the dropdowns inaccessible to screen readers, as the association is strictly visual.
**Action:** Always include an explicit `aria-label` attribute on the `<select>` element to ensure proper context is announced by screen readers when `htmlFor` and `id` are not used.
