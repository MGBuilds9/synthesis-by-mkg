...
**Learning:** Empty states displaying missing data (e.g., "No emails yet") without direct action links cause user friction, forcing them to manually navigate to settings.
**Action:** Always include an actionable `<Link>` directly in empty state messages pointing to the relevant resolution page (e.g., `/dashboard/settings`).
