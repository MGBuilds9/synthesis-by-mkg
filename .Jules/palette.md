...
## 2026-08-21 - Accessible Select Elements
**Learning:** Visual labels placed next to <select> elements without an explicit htmlFor association are not announced correctly by screen readers.
**Action:** Always ensure <select> elements have an explicit aria-label or are properly linked to a <label> with htmlFor and id to maintain accessibility.
