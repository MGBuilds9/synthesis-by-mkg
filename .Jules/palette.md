...
## 2026-08-23 - Select element accessibility
**Learning:** Select elements that rely on visual contextual placement next to text nodes (rather than proper `<label htmlFor>`) are inaccessible to screen readers as they lack explicit associations.
**Action:** Always provide an explicit `aria-label` on `<select>` dropdowns when a linked label element with an `id` is not present, ensuring their purpose is announced by screen readers.
