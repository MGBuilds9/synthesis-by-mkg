...
## 2026-08-03 - Add Loading Spinner to Async Buttons

**Learning:** When async operations like searching are indicated only by text changes (e.g. "Search" -> "Searching..."), it can be subtle. Adding an explicit loading spinner icon alongside the text, combined with appropriate `aria-busy` attributes and visible disabled states, greatly enhances user feedback and accessibility.
**Action:** Always pair loading text with a visual spinner (e.g. `Loader2` with `animate-spin` and `aria-hidden="true"`) and ensure disabled buttons have clear visual states (`opacity-50`, `cursor-not-allowed`).
