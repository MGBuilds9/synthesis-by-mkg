...
## 2026-08-20 - Unassociated Select Labels
**Learning:** Visual labels placed next to `<select>` dropdowns do not automatically associate with the input for screen readers without `htmlFor` and `id` bindings.
**Action:** Always include an explicit `aria-label` attribute on `<select>` elements when they lack a programmatically associated label to ensure proper context is announced.
