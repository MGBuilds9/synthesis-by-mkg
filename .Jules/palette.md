...
## 2026-05-25 - [Conditional aria-controls]
**Learning:** When elements rendered conditionally are targeted by `aria-controls`, setting the attribute unconditionally can reference a non-existent DOM ID when the element is closed, causing accessibility issues.
**Action:** Always wrap `aria-controls` in the same logic used to conditionally render the target element (e.g., `aria-controls={isOpen ? 'panel-id' : undefined}`).
