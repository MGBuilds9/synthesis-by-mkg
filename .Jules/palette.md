...
## 2026-08-17 - Select Element Labels
**Learning:** Visual labels placed near a `<select>` dropdown do not inherently provide an accessible name unless linked via `htmlFor` and `id`.
**Action:** When visual labels are not directly linked to a `<select>`, always include an explicit `aria-label` on the `<select>` element.
