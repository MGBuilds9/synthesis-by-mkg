...
## 2026-09-19 - Prevent button layout shift during async operations
**Learning:** Adding a loading spinner next to text in a button can cause annoying layout jitter if the button's width isn't constrained, as the text + spinner takes up more space than the original text.
**Action:** Always pair minimum width utilities (like `min-w-[140px]`) and flexbox centering when introducing visual loading spinners inside interactive buttons to ensure a smooth, stable experience.
