...
## 2026-08-10 - Filter Select Accessibility
**Learning:** Filter controls often use visual labels alongside <select> dropdowns, but if they lack `htmlFor` and an `id`, screen reader users lose context on what the dropdown controls.
**Action:** Always include an explicit `aria-label` attribute on the <select> element if it is not explicitly tied to a label tag via `htmlFor`/`id` pairings.
