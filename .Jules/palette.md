...
## 2026-08-09 - Unassociated Form Labels
**Learning:** Visual labels placed next to `<select>` dropdowns without using `htmlFor` and an `id` or an `aria-label` are not announced correctly by screen readers, hiding context.
**Action:** Always ensure form elements have a semantic association with their labels, either via `htmlFor`/`id` pairing or an explicit `aria-label`.
