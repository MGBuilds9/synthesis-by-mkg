...
## 2026-08-22 - Add aria-label to Select Elements
**Learning:** Select elements functioning as dropdowns lack context for screen readers when they do not have an explicit `aria-label`.
**Action:** Always add an explicit `aria-label` to `<select>` dropdowns to ensure screen readers can announce their purpose.
