...
## 2026-08-25 - Add aria-labels to select elements
**Learning:** Adding visual labels next to `<select>` dropdowns without using `htmlFor` and an `id` makes the dropdown inaccessible to screen readers because they cannot programmatically associate the label.
**Action:** Always include an explicit `aria-label` attribute on `<select>` elements when a programmatic `<label htmlFor="...">` association is missing.
