...
## 2026-08-12 - Select Input Accessibility
**Learning:** Visually paired text labels next to `<select>` elements are not inherently associated for screen readers unless explicitly linked via `htmlFor` and `id`.
**Action:** Always explicitly provide an `aria-label` on `<select>` elements if they are not explicitly linked to a `<label>` or if the linkage is missing to ensure they are accessible.
