...
## 2026-08-07 - Focus States on Card-like Buttons
**Learning:** Custom interactive elements (like the "Connect Account" grid buttons) that use border-color changes for hover states often fail to clearly indicate keyboard focus because the browser's default outline blends in or gets overridden.
**Action:** Always add explicit `focus:outline-none focus-visible:ring-2 focus-visible:ring-[color] focus-visible:ring-offset-[size]` utility classes to custom interactive grid buttons to ensure keyboard users can clearly see their focus state.
