...
## 2026-08-01 - Add loading spinner to async submit button
**Learning:** Providing clear visual feedback (like a loading spinner) during asynchronous operations improves user perception of performance and prevents duplicate clicks.
**Action:** Always include a visual loading indicator (e.g., `Loader2` with `animate-spin`) inside submit buttons when they trigger async actions, along with disabling the button and setting `aria-busy`.
