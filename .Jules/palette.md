...
## 2026-08-29 - Select Accessibility
**Learning:** Visual labels next to `<select>` dropdowns do not automatically associate with the dropdown if `htmlFor` and an `id` are not used, making them inaccessible to screen readers.
**Action:** Always include an explicit `aria-label` attribute on the `<select>` element if it cannot be linked directly to its visual label.
