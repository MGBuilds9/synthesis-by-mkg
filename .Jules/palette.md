...
## 2026-07-21 - Interaction Feedback (Search Button)
**Learning:** The storage search button lacked a loading state, leaving users without visual feedback when clicking "Search".
**Action:** Standardize async button feedback by replacing static text with a `Loader2` icon, adding a spinning animation, and using disabled tailwind states (`disabled:opacity-50 disabled:cursor-not-allowed`) to indicate processing.
