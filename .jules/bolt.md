...
## 2026-08-03 - Cached LLM Provider Instances
**Learning:** Instantiating external SDK clients (like OpenAI or Anthropic) on every API request prevents the reuse of underlying HTTP keep-alive agents, causing unnecessary TLS handshake overhead and increasing response latency.
**Action:** Cache and reuse SDK client instances at the module level across requests to leverage persistent HTTP connections and reduce Time To First Byte (TTFB).
