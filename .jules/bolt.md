...
## 2026-05-26 - Array includes to Set has
**Learning:** Using array literals for membership checks in high-traffic callbacks (like `Array.filter` in Next.js API routes) causes redundant object allocations and garbage collection overhead during every request execution.
**Action:** Extract these into `Set` constants at the module level.
