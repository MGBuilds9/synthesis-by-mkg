...
## 2026-08-30 - Caching SDK Instances
**Learning:** Instantiating external SDK clients (like OpenAI or Anthropic) on every request wastes time on TLS handshakes/connection setups, creating performance bottlenecks.
**Action:** Cache these instances at the module level using a `Map` or separate variables so that multiple requests can reuse the same underlying HTTP keep-alive agents, significantly reducing TTFB.
