...
## 2026-08-31 - LLM Provider Caching
**Learning:** Instantiating external SDK client instances (e.g., OpenAI, Anthropic) per-request in serverless/Node.js environments prevents the reuse of underlying HTTP keep-alive agents, significantly increasing TLS handshake latency (TTFB).
**Action:** Cache external SDK client instances at the module level to reuse HTTP keep-alive agents. Always expose a reset function (e.g., `resetProviderCache()`) and call it within the `beforeEach` block of relevant test suites to prevent state leakage across tests.
