...
## 2026-08-25 - Caching SDK Client Instances
**Learning:** Instantiating external SDK clients (like OpenAI or Anthropic) per request in serverless/Node.js environments adds significant TLS handshake overhead.
**Action:** Cache these client instances at the module level to reuse underlying HTTP keep-alive agents, improving Time to First Byte (TTFB). Always provide a `resetProviderCache` function for tests to avoid state leakage.
