...
## 2026-08-13 - Cache External SDK Instances
**Learning:** Instantiating external SDK clients (like OpenAI or Anthropic) on every request in serverless/Node.js environments incurs unnecessary TLS handshake and connection overhead, reducing performance (TTFB).
**Action:** Cache these external SDK client instances at the module level to reuse underlying HTTP keep-alive agents across multiple requests.
