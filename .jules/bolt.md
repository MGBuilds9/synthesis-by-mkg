...
## 2026-08-01 - Caching External SDK Client Instances
**Learning:** Instantiating external SDK clients (like OpenAI or Anthropic) per-request prevents the reuse of underlying HTTP keep-alive agents, causing significant TLS handshake latency (TTFB).
**Action:** Cache these SDK client instances at the module level. Always expose and call a reset function like `resetProviderCache()` within test suites' `beforeEach` to prevent state leakage.
