...
## 2026-09-13 - Combine Array Iterations
**Learning:** Chaining array methods like `.map()` sequentially on the same list causes multiple redundant iterations over the dataset.
**Action:** When deriving multiple data structures (like an ID array and a Map) from the same list, use a single `for...of` loop or `reduce` to process elements in a single pass.
