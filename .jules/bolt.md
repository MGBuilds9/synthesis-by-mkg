...
## 2026-08-13 - Cache LLM Providers to avoid unnecessary initializations
**Learning:** Creating a new LLM provider instance on every request introduces unnecessary latency because it recreates connections.
**Action:** Cache the external SDK client instances (e.g. OpenAI, Anthropic) at the module level rather than instantiating them per-request.
