...
## 2026-08-24 - Module-Level Caching for LLM Providers
**Learning:** Re-instantiating external SDK clients (like OpenAI or Anthropic) on every request causes performance overhead due to repeated TLS handshakes.
**Action:** Cache these provider instances at the module level to reuse the underlying HTTP keep-alive agents, which significantly reduces request latency (TTFB) in serverless/Node.js environments. Always expose and use a `resetProviderCache()` function in test setup to prevent state leakage.
