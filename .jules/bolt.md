...
## 2026-08-15 - Cache external SDK client instances at the module level
**Learning:** Instantiating LLM SDK clients (like OpenAI, Anthropic, Gemini) on every request causes unnecessary overhead and prevents the reuse of underlying HTTP keep-alive agents, significantly increasing TLS handshake latency (TTFB).
**Action:** Cache external SDK client instances at the module level rather than instantiating them per-request.
