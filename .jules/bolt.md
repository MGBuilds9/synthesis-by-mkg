...
## 2026-08-27 - Cache SDK clients to reuse HTTP agents
**Learning:** In serverless API routes, creating a new instance of an LLM SDK client (like OpenAI or Anthropic) on every request prevents the reuse of underlying HTTP keep-alive agents, leading to significant TLS handshake latency (TTFB) on every API call.
**Action:** Always cache SDK client instances at the module level in Node.js/serverless environments. Provide a reset function (e.g. `resetProviderCache()`) and call it in test `beforeEach` hooks to prevent state leakage between unit tests.
