...
## 2026-08-07 - Cache external SDK instances at module level
**Learning:** Instantiating external SDK clients per-request causes unnecessary TLS handshake latency and prevents reuse of HTTP keep-alive agents, significantly degrading TTFB.
**Action:** Cache external SDK clients (like OpenAI, Gemini, Claude) at the module level (e.g., using a Map or singleton pattern) instead of creating new instances inside the provider factory function.
