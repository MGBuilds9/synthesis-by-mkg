...
## 2026-06-05 - Avoid Unnecessary Counts in Frontend Lookups
**Learning:** Adding the \`includeCount=false\` URL parameter in fetch calls explicitly skips backend database count aggregations, saving time and compute resources when the UI doesn't use the pagination count directly.
**Action:** When components fetch data lists but only render the items (without total counts), ensure fetch URLs specify \`includeCount=false\`.
