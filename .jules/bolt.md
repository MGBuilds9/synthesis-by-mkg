...
## 2026-05-11 - React latest-ref pattern for stable callbacks
**Learning:** In Next.js/React, components wrapped in `React.memo()` can still suffer from unnecessary re-renders if they are passed inline arrow functions or unstable callbacks as props (e.g., `onSuggestionClick={(text) => sendMessage(text)}`). If the callback itself depends on frequently changing state, you cannot just wrap it in `useCallback` directly without bringing in those state dependencies, which invalidates the callback anyway.
**Action:** Use the "latest-ref" pattern. Store the frequently-changing function in a `useRef` that gets updated via `useEffect` after every render. Then, create a stable callback with `useCallback` (with empty dependencies) that calls the function stored in the ref. Pass this stable callback to the memoized child component to prevent it from re-rendering when the parent's state changes.
