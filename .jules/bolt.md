...
## 2026-08-24 - Cache external SDK client instances
**Learning:** Instantiating external SDK clients (like OpenAI or Anthropic) per-request introduces significant TLS handshake latency (TTFB) and prevents the reuse of underlying HTTP keep-alive agents.
**Action:** Always cache external SDK client instances at the module level rather than instantiating them per-request in serverless/Node.js environments.
