...
## 2026-08-12 - Optimize LLM Provider Initialization
**Learning:** Instantiating new LLM providers (and their underlying SDK clients) on every chat request introduces unnecessary memory allocation and setup overhead.
**Action:** Cache external SDK client instances at the module level (e.g., using a Map) so they are reused across requests, reducing TLS handshake latency (TTFB) in serverless/Node.js environments.
