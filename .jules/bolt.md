...
## 2026-08-01 - Cache External SDK Instances
**Learning:** Instantiating external SDK clients (like OpenAI, Anthropic) per-request in serverless/Node.js environments incurs significant TLS handshake latency (TTFB) because underlying HTTP keep-alive agents cannot be reused.
**Action:** Cache these SDK client instances at the module level. Always expose a reset function (e.g., resetProviderCache) and call it within the beforeEach block of relevant test suites to prevent state leakage across tests.
