...
## 2026-09-17 - Prevent Unnecessary Re-renders and Memory Allocation for Static State
**Learning:** Initializing static object or array literals directly within `useState` inside a component body re-evaluates and re-allocates memory on every render. Furthermore, using `useEffect` to synchronously set static or mock initial state triggers an unnecessary second render cycle and delays Time to Interactive (TTI).
**Action:** Extract static data into constants outside the component definition to ensure a single instance is shared, and initialize state directly in the `useState` hook instead of relying on an effect.
