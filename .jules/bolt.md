...
## 2026-08-18 - LLM Provider Caching
**Learning:** Instantiating external SDK clients (e.g., OpenAI, Anthropic) per-request instead of caching them at the module level introduces significant TLS handshake latency (TTFB) because the underlying HTTP keep-alive agents cannot be reused across requests.
**Action:** Cache external SDK client instances at the module level (e.g., using a Map) and export a `resetProviderCache()` function to be called in `beforeEach` blocks of relevant test suites to prevent state leakage. Update test file mocks to include the new reset function to avoid ReferenceErrors.
