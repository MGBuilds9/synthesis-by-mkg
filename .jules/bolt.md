...
## 2026-08-14 - Reusing External SDK Clients for Connection Pooling
**Learning:** Instantiating external SDK clients (like OpenAI, Anthropic) per-request prevents the reuse of underlying HTTP keep-alive agents, increasing TLS handshake latency (TTFB) for every API call.
**Action:** Cache SDK client instances at the module level in serverless/Node.js environments rather than instantiating them per-request to optimize network performance.
