...
## 2026-08-08 - Cache LLM provider instances
**Learning:** Instantiating external SDK clients (like OpenAI, Anthropic) per-request in serverless or Node.js environments (like Next.js API routes) creates unnecessary TLS handshakes for every API call, leading to poor Time To First Byte (TTFB).
**Action:** Always cache these client instances at the module level using a Map or similar construct so they can reuse underlying HTTP keep-alive connections.
