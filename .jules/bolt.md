...
## 2026-08-15 - Cache SDK Clients to Optimize API Integrations
**Learning:** Instantiating external SDK clients (e.g., OpenAI, Anthropic) per-request prevents the reuse of underlying HTTP keep-alive agents and significantly increases TLS handshake latency (TTFB).
**Action:** Cache external SDK client instances at the module level in serverless/Node.js environments rather than instantiating them per-request. This optimization improves connection pooling and TTFB latency.
