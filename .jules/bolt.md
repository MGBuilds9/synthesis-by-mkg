...
## 2026-08-25 - Module-Level SDK Caching
**Learning:** Instantiating external SDK clients (like OpenAI or Anthropic) on every request causes unnecessary TLS handshake latency and prevents the reuse of underlying HTTP keep-alive agents in serverless/Node.js environments.
**Action:** Cache these external SDK client instances at the module level so they are reused across invocations, but ensure a reset function is provided and called in tests to prevent state leakage.
