...
## 2026-09-01 - Optimizing LLM Provider Instantiation
**Learning:** Instantiating external SDK clients (like OpenAI, Anthropic) per-request prevents reuse of underlying HTTP keep-alive agents, leading to unnecessary TLS handshake overhead and increased latency (TTFB).
**Action:** Cache these SDK client instances at the module level rather than per-request, and expose a cache reset function for isolated testing.
