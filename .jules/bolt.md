...
## 2026-07-27 - Optimize AI Chat Rate Limiting
**Learning:** Using database `count` queries with nested joins for high-frequency operations like rate limiting adds unnecessary latency and database load.
**Action:** Use an in-memory sliding window `RateLimiter` for O(1) rate limiting to improve performance without compromising constraints.
