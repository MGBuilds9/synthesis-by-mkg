...
## 2026-08-11 - Optimizing External API Client Instantiation
**Learning:** In serverless/Node.js environments, instantiating new external SDK clients (like OpenAI, Anthropic) per request adds significant overhead due to TLS handshakes and missing HTTP keep-alive reuse.
**Action:** Cache the instantiated clients at the module level so subsequent requests in the same lambda/process can reuse the underlying HTTP connections, reducing TTFB latency.
