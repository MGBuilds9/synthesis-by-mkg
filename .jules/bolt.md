...
## 2026-09-02 - Cache LLM Providers
**Learning:** Instantiating external SDK clients per-request significantly increases TLS handshake latency (TTFB) in serverless/Node.js environments.
**Action:** Always cache external SDK clients at the module level. Ensure to provide a `resetProviderCache` function and call it within tests to prevent state leakage.
