...
## 2026-08-10 - Cache LLM provider instances
**Learning:** Instantiating external SDK clients (like OpenAI, Anthropic) on every request introduces significant overhead due to repeated TLS handshakes and missing HTTP keep-alive agent reuse in Node.js serverless environments.
**Action:** Always cache these instances at the module level rather than creating them per-request to improve TTFB.
