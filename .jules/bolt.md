...
## 2026-08-20 - Module-Level LLM Provider Instance Caching
**Learning:** Instantiating external SDK clients (like OpenAI or Anthropic) on every API request in a serverless/Node.js environment introduces significant overhead due to repeated TLS handshakes and connection setup. Reusing the HTTP keep-alive agents can reduce this TTFB latency.
**Action:** Implemented caching for LLM provider instances at the module level in lib/providers/llm/index.ts. Used a Map to store previously instantiated providers, ensuring connections are reused across subsequent requests. Also provided a resetProviderCache() method to clear the cache during tests.
