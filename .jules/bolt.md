...
## 2026-09-04 - Avoid Unnecessary Relation Joins on Create
**Learning:** When creating a new entity (like an AI Chat Session) using Prisma, applying an `include` block to fetch relations causes an unnecessary database join operation, even though the relations are guaranteed to be empty initially.
**Action:** Remove `include` from `create` statements where possible and manually initialize the relation arrays (e.g., `session.messages = []`) in code to satisfy type constraints without the DB overhead.
