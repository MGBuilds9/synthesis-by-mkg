...
## 2026-05-25 - Bypass Empty Prisma In Queries
**Learning:** When querying Prisma with an `in` operator (e.g., `where: { id: { in: ids } }`), if the target ID array is empty, Prisma still executes an unnecessary database query.
**Action:** Optimize this by bypassing the query conditionally (e.g., `const results = ids.length > 0 ? await db.query() : []`) instead of executing it with an empty array or adding an early return that duplicates API response logic.
