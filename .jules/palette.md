...
**Learning:** Empty states showing "No [items] yet" without a direct link to resolve the issue leave users stuck.
**Action:** Always include an actionable `<Link href="/dashboard/settings">` (since there are no dedicated integration pages) within empty states to guide users.
