...
## 2026-03-12 - Actionable Empty States
**Learning:** Empty states that simply inform users of missing data (e.g., "No emails yet") create friction. Users are forced to manually find the right page to resolve the state, which breaks their flow and can lead to drop-off.
**Action:** Always design empty states to be actionable. Include a clear `<Link>` or call-to-action button that points directly to the relevant settings or connection page so the user can immediately resolve the missing state.
