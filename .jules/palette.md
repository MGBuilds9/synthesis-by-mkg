...
**Learning:** Using a static `aria-controls` attribute for a conditionally rendered target element creates an accessibility violation when the target is not in the DOM.
**Action:** Always conditionally map `aria-controls` to `undefined` when the targeted panel or element is hidden/unrendered (e.g., `aria-controls={isOpen ? 'panel-id' : undefined}`).
