...
## 2026-05-24 - Accessibility: aria-controls conditionally rendered elements
**Learning:** When using `aria-expanded` on a button to reveal a panel, if the panel is conditionally rendered in React and might not exist in the DOM, the `aria-controls` attribute should also be conditionally applied to match the exact rendering logic (or omitted/set to undefined when unmounted) to prevent pointing to a non-existent ID.
**Action:** Always verify that `aria-controls` points to a valid ID in the current DOM, especially when the controlled element's render state depends on complex compound conditions.
