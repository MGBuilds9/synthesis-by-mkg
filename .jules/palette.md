...
**Learning:** Empty states that simply inform users of missing data (e.g., "No emails yet") without providing a direct path to resolution create friction.
**Action:** Always include an actionable `<Link>` in empty states pointing directly to the relevant settings or configuration page (like `/dashboard/settings`) so users can immediately resolve the state.
