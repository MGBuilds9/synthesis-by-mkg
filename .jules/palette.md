...
## 2026-06-21 - Dynamic aria-controls for conditional elements
**Learning:** Hardcoding `aria-controls` for elements that conditionally render violates accessibility IDREF validation when the target element is absent from the DOM.
**Action:** Conditionally apply `aria-controls` using `{isOpen ? 'panel-id' : undefined}` to prevent pointing to non-existent elements.
