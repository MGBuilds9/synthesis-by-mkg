...
## 2026-06-21 - Conditional aria-controls for Toggles
**Learning:** Hardcoding `aria-controls` attributes to point to elements that are conditionally rendered in the DOM results in broken references when the element is hidden, leading to IDREF validation errors and confusing screen reader output.
**Action:** Always conditionally apply `aria-controls` attributes (e.g., `aria-controls={isOpen ? 'panel-id' : undefined}`) so they are only present when the target element exists in the DOM.
