...
## 2023-10-24 - Loading State Layout Shifts and Accessibility
**Learning:** Text-only loading states (like swapping "Search" to "Searching...") inside buttons can cause noticeable visual layout shifting due to text width differences. Furthermore, updating the text content of a disabled button isn't sufficient for screen readers without explicitly using `aria-busy="true"` or an accessible status role. When updating empty states in main content areas to indicate loading, using a visual spinner without a `role="status"` and a hidden text label (like `sr-only`) leaves screen reader users completely unaware that background activity is occurring.
**Action:** When adding async operation visual feedback, always pair a visual spinner (like `Loader2` from lucide-react) with a `min-w-[]` class on the parent button to prevent jitter. Explicitly mark loading empty states with `role="status"` and an accessible `sr-only` text span, while hiding the decorative visible text from screen readers with `aria-hidden="true"`.
