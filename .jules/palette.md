...
## 2026-07-05 - Aria-controls on conditionally rendered panels
**Learning:** Hardcoding `aria-controls` attributes that target conditionally rendered elements (e.g. `aria-controls="panel-id"`) results in accessibility violations when the element is removed from the DOM, as the screen reader looks for a non-existent ID.
**Action:** Dynamically set `aria-controls={isOpen ? 'panel-id' : undefined}` for conditionally rendered targets so that the attribute is removed when the element is not in the DOM.
