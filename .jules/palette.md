...
## 2026-06-24 - Conditional ARIA Controls
**Learning:** Using static strings for `aria-controls` on conditionally rendered elements triggers IDREF validation errors when the target element is absent from the DOM.
**Action:** Always conditionally apply `aria-controls={isOpen ? 'panel-id' : undefined}` when linking a toggle button to a collapsible panel that may not be mounted.
