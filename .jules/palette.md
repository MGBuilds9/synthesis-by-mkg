...
## 2026-07-02 - Dynamic aria-controls
**Learning:** Hardcoded `aria-controls` attributes referencing conditionally rendered DOM elements result in accessibility violations when the target is absent from the DOM.
**Action:** Always ensure that `aria-controls` targeting conditionally rendered elements dynamically resolves to `undefined` when the target is not rendered (e.g. `aria-controls={isOpen ? 'panel-id' : undefined}`).
