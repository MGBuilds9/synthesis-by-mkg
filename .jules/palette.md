...
## 2026-06-25 - Conditional ARIA Controls
**Learning:** Elements using `aria-controls` to point to conditionally rendered IDs violate accessibility IDREF validation when the panel is hidden (not in the DOM).
**Action:** Always apply `aria-controls` conditionally (e.g., `aria-controls={isOpen ? 'panel-id' : undefined}`) rather than as a static string when the target panel is removed from the DOM.
