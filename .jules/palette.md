...
## 2026-09-14 - Async Button Layout Jitter
**Learning:** Changing button text during async operations (e.g., from "Search" to "Searching...") often causes layout jitter due to width changes, which is visually jarring.
**Action:** Always apply a minimum width utility class (e.g., `min-w-[140px]`) alongside flex centering when a button's content changes dynamically during async operations to maintain layout stability.
