...
## 2026-06-20 - Conditional aria-controls for Dynamic Panels
**Learning:** Hardcoding `aria-controls` attributes on toggle buttons when the target panel is conditionally rendered in the DOM results in IDREF validation errors and confuses screen readers.
**Action:** Always conditionally apply `aria-controls` based on the panel's visibility state (e.g., `aria-controls={isOpen ? 'panel-id' : undefined}`).
