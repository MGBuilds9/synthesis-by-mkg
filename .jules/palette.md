...
## 2026-06-23 - Conditional aria-controls for Modals/Panels
**Learning:** Hardcoding `aria-controls` attributes to statically reference IDs of conditionally rendered DOM elements (like panels or dropdowns) causes IDREF validation errors and confuses assistive technologies when the target element isn't in the DOM.
**Action:** Always conditionally apply the `aria-controls` attribute (e.g., `aria-controls={isOpen ? 'panel-id' : undefined}`) so it is only present when the target element is actually rendered.
