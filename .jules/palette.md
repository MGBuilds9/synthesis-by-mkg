...
## 2026-07-04 - Conditional Aria-Controls Validation
**Learning:** Hardcoding `aria-controls` attributes to point to DOM IDs of elements that are conditionally rendered causes screen readers to announce relationships to non-existent elements when the target is unmounted.
**Action:** Always ensure that `aria-controls` attributes targeting conditionally rendered elements (e.g., collapsible panels) are dynamically set to `undefined` when the target is not rendered in the DOM (e.g., `aria-controls={isOpen ? 'panel-id' : undefined}`).
