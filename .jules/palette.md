...
## 2026-07-11 - Dynamic aria-controls
**Learning:** Setting static `aria-controls` attributes for conditionally rendered elements (like filter panels) violates accessibility rules when the target element is not in the DOM.
**Action:** Always dynamically evaluate `aria-controls` to `undefined` when the target element is not rendered (e.g., `aria-controls={isOpen ? 'panel-id' : undefined}`).
