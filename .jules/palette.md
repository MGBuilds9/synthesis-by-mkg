...
## 2026-06-18 - Conditional Aria Controls
**Learning:** Hardcoding `aria-controls` attributes to elements that are conditionally rendered in the DOM can result in broken ARIA references when the elements are hidden.
**Action:** When using `aria-controls` to link a toggle button to a conditionally rendered panel, always conditionally apply the `aria-controls` attribute (e.g. `aria-controls={isOpen ? 'panel-id' : undefined}`).
