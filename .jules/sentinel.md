...
## 2026-06-14 - Unhandled NaN in Pagination Offset
**Vulnerability:** The `offset` parameter in pagination APIs was parsed with `parseInt` and passed directly to the database layer without checking for `NaN`. This could result in passing `NaN` to Prisma's `skip` method, causing unhandled exceptions and potential Denial of Service (DoS).
**Learning:** `parseInt` returns `NaN` for invalid inputs. When passing parsed numeric query parameters directly to database ORM methods, failing to validate and fallback from `NaN` can trigger unexpected server-side errors.
**Prevention:** Always validate parsed numbers using `isNaN()` and enforce proper bounds (e.g., using `Math.max(0, parsedValue)`) before passing them to backend services or database queries.
