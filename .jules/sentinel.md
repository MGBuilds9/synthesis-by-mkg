...
## 2026-06-30 - Denial of Service via Unvalidated Prisma Query Parameters
**Vulnerability:** Prisma queries crashed, causing a Denial of Service, when `NaN` was passed to the `skip` clause because `parseInt` results were not validated for non-numeric input on the `offset` parameter.
**Learning:** When parsing numeric inputs from URL query parameters (e.g., using `parseInt`), the result can be `NaN`. If passed unvalidated to database ORMs like Prisma, this causes unhandled exceptions.
**Prevention:** Always validate parsed numeric inputs using `isNaN()` and provide safe fallback values before passing them to database operations.
