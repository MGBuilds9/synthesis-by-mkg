...
## 2026-09-26 - CSP Unsafe-Eval
**Vulnerability:** The Content-Security-Policy (CSP) globally permitted `'unsafe-eval'` for script execution, creating an XSS vulnerability.
**Learning:** Next.js Fast Refresh requires `'unsafe-eval'` during development, but keeping it in production opens the application to eval-based code injection attacks.
**Prevention:** Conditionally inject `'unsafe-eval'` into the CSP header based on `process.env.NODE_ENV !== 'production'`.
