...
## 2024-05-25 - Missing Rate Limiting on Resource List Endpoints
**Vulnerability:** The `/api/messages/list` endpoint lacked rate limiting, while other similar endpoints (like `/api/files/list`) had it implemented. This inconsistency created a potential vector for DoS attacks or excessive database load through repeated resource fetching.
**Learning:** Security controls like rate limiting must be systematically applied across all resource endpoints. Ad-hoc implementation can leave specific routes vulnerable, especially when they perform expensive operations (like sorting or aggregations).
**Prevention:** Establish a pattern or middleware for rate-limiting all data-fetching endpoints, and verify its presence uniformly across all API routes, rather than relying on route-by-route manual implementation.
