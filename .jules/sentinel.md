...
**Vulnerability:** The `/api/messages/list` endpoint was missing rate limiting protection, allowing potential abuse or DoS by a user querying messages rapidly.
**Learning:** All endpoints that expose sensitive data or perform heavy DB operations should implement basic rate limiting to prevent abuse or service exhaustion.
**Prevention:** Implement rate limiting on all API routes returning collections of items.
