...
## 2026-09-12 - Prevent Plaintext Credential Logging in UI Components
**Vulnerability:** The client-side sign-in form explicitly logged the user's email and plaintext password to the browser console.
**Learning:** Even placeholder or demonstrative code can inadvertently leak highly sensitive credentials to the browser console, which is a major security risk for users in shared environments or when interacting with malicious browser extensions.
**Prevention:** Never use `console.log` or other logging mechanisms to output sensitive user inputs (passwords, tokens, API keys) in frontend code. Use proper authentication flow libraries (like NextAuth) and restrict logging to safe, non-sensitive context.
