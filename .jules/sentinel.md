...
## 2026-09-06 - Remove unsafe-eval from production CSP
**Vulnerability:** The Content-Security-Policy globally allowed `unsafe-eval` in the `script-src` directive, which enables eval-based XSS attacks in production.
**Learning:** Next.js development (specifically Webpack HMR/Fast Refresh) relies on `unsafe-eval`. Stripping it unconditionally breaks local development.
**Prevention:** Conditionally construct the CSP `script-src` directive by checking `process.env.NODE_ENV === 'production'` so that `unsafe-eval` is only present in development.
