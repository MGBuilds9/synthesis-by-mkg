...
## 2026-08-30 - Conditional CSP Directives
**Vulnerability:** The Content-Security-Policy allowed 'unsafe-eval' in the script-src directive unconditionally, which increases the risk of Cross-Site Scripting (XSS) in production environments.
**Learning:** While development environments like Next.js require 'unsafe-eval' for Fast Refresh (Webpack HMR), it is not needed and poses a significant security risk when deployed to production.
**Prevention:** Conditionally construct CSP headers to include development-only directives (like 'unsafe-eval') only when process.env.NODE_ENV !== 'production'.
