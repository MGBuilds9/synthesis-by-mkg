...
## 2026-09-17 - Remove unsafe-eval from production CSP
**Vulnerability:** The Content-Security-Policy (CSP) allowed 'unsafe-eval' universally, exposing production to Cross-Site Scripting (XSS) risks.
**Learning:** Next.js requires 'unsafe-eval' for Fast Refresh during development, but this must be conditionally removed in production.
**Prevention:** Conditionally inject 'unsafe-eval' based on `process.env.NODE_ENV !== 'production'`.
