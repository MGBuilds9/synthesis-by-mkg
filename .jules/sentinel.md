...
## 2026-06-27 - Unhandled NaN input in Pagination Offset
**Vulnerability:** The API endpoints `/api/messages/list` and `/api/files/list` failed to explicitly validate the parsed `offset` query parameter before passing it to the Prisma `skip` argument. When an invalid non-numeric offset was provided, `parseInt` returned `NaN`, resulting in unhandled query exceptions and potential Denial of Service (HTTP 500 errors).
**Learning:** External inputs parsed as numbers (like pagination parameters) can result in `NaN`. Prisma and other ORMs often fail when receiving `NaN` for numeric arguments. Relying on `|| '0'` during parsing is insufficient, as `parseInt('invalid')` returns `NaN`, which is truthy in JavaScript context.
**Prevention:** Always explicitly validate parsed numeric inputs using `isNaN()` and apply safe fallbacks (e.g., `Math.max(0, isNaN(rawOffset) ? 0 : rawOffset)`) before passing them to ORM methods or executing business logic.
