...
## 2026-09-12 - CSP `unsafe-eval` Restriction
**Vulnerability:** The Content Security Policy in `middleware.ts` unconditionally allowed `unsafe-eval` in `script-src`. While necessary for development tools (like Next.js Fast Refresh), it allows attackers to execute arbitrary code via functions like `eval()` or `setTimeout(string)` if an XSS vulnerability exists.
**Learning:** Security directives required for development environments must be strictly conditionally removed for production builds.
**Prevention:** Conditionally construct CSP headers using environment checks (e.g., `process.env.NODE_ENV === 'production'`) to enforce stricter rules when the application is deployed.
