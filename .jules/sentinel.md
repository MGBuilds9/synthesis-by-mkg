...
**Vulnerability:** The Content-Security-Policy unconditionally allowed 'unsafe-eval' in script-src, enabling potential Cross-Site Scripting (XSS) via eval() or Function().
**Learning:** While 'unsafe-eval' is required by Next.js for Fast Refresh during local development, it must not be leaked into production environments.
**Prevention:** Conditionally construct the CSP header string based on process.env.NODE_ENV, appending 'unsafe-eval' only when not in 'production'.
