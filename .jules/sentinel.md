...
## 2026-09-22 - Fix XSS Risk via unsafe-eval in Content-Security-Policy
**Vulnerability:** The Content-Security-Policy's `script-src` unconditionally allowed `'unsafe-eval'`.
**Learning:** Next.js requires `'unsafe-eval'` for Fast Refresh in development mode, leading developers to sometimes add it globally and accidentally leave it enabled in production, creating an XSS vulnerability vector.
**Prevention:** Always conditionally append development-only relaxations like `'unsafe-eval'` to the CSP based on `process.env.NODE_ENV !== 'production'`.
