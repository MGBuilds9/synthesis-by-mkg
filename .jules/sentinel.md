...
## 2026-09-14 - Conditional CSP unsafe-eval
**Vulnerability:** Content-Security-Policy included `'unsafe-eval'` unconditionally in the `script-src` directive, allowing arbitrary string execution (like `eval()`) which could lead to XSS in production.
**Learning:** Next.js development server requires `'unsafe-eval'` for Fast Refresh, meaning we cannot entirely remove it but must conditionally apply it based on `process.env.NODE_ENV !== 'production'`.
**Prevention:** Always scope developmental requirements like `'unsafe-eval'` to development environments conditionally, explicitly removing them in production payloads to maintain strict CSP hygiene.
