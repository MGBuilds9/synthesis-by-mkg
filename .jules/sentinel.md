...
## 2026-04-20 - Unstructured Logging Leakage\n**Vulnerability:** Use of raw console.error in backend code bypassed the structured logging system (Winston), potentially leaking unredacted/unformatted stack traces to unstructured production logs.\n**Learning:** Relying on console.error instead of the centralized logger undermines logging infrastructure and visibility while introducing data leakage risks.\n**Prevention:** Use the centralized logger (e.g., logger.error) everywhere in the backend and ensure the error object is correctly formatted ({ error }).
