...
## 2026-07-04 - Unhandled Prisma Query Exception (NaN in Skip/Take)
**Vulnerability:** Passing unvalidated parsed integer parameters (e.g., from `parseInt('abc')` resulting in `NaN`) directly into Prisma's `skip` or `take` methods causes unhandled query exceptions and results in HTTP 500 Denial of Service errors.
**Learning:** The `parseInt` function returns `NaN` for non-numeric strings. Prisma expects valid integers for `skip` and `take`, and errors out when receiving `NaN`, exposing backend crashes to users if not caught or validated.
**Prevention:** Always explicitly check for `NaN` (e.g., using `isNaN()`) when parsing URL query parameters for numeric Prisma inputs, and provide a safe fallback value (e.g., `0` for `skip`, `50` for `take`).
