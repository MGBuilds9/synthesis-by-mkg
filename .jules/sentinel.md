...
## 2026-08-07 - Prevent Plaintext Credential Logging
**Vulnerability:** The sign-in page logged plaintext passwords to the client console using `console.log('Sign in with:', { email, password })`.
**Learning:** Client-side logging can expose sensitive credentials to anyone with access to the browser developer tools, presenting a critical security risk.
**Prevention:** Never log passwords, tokens, or other sensitive authentication materials, even in development environments or commented out "TODO" blocks.
