...
## 2026-07-01 - Unhandled NaN in Prisma Query Parameters
**Vulnerability:** The API endpoints parsed pagination offsets using `parseInt` without validating the result against `NaN` before passing it to Prisma's `skip` method. This allows attackers to send non-numeric input (e.g., `?offset=abc`), resulting in unhandled query exceptions and potential Denial of Service (500 errors).
**Learning:** `parseInt` returns `NaN` for invalid string inputs, which Prisma methods (like `skip` or `take`) do not handle safely, leading to application crashes.
**Prevention:** Always validate parsed numeric inputs using `isNaN()` and provide safe fallback values (e.g., `Math.max(0, isNaN(rawOffset) ? 0 : rawOffset)`) before passing them to database queries.
