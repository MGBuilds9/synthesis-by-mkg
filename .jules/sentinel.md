...
## 2026-09-16 - Prevent Invalid Pagination Input (Denial of Service)
...
## 2026-09-16 - Conditional CSP Directives
**Vulnerability:** The Content-Security-Policy (CSP) allowed `'unsafe-eval'` in production environments, increasing the risk of Cross-Site Scripting (XSS).
**Learning:** While `'unsafe-eval'` is required by Next.js for Fast Refresh during development, it should never be permitted in a production `script-src` directive as it enables execution of arbitrary strings as code.
**Prevention:** Always conditionally construct CSP headers based on `process.env.NODE_ENV` to ensure strict policies in production without breaking development workflows.
