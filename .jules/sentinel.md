...
## 2026-07-07 - Unhandled Prisma Query Exception via Invalid Offset Input
**Vulnerability:** Passing unvalidated URL query parameters to Prisma's `skip` or `take` methods when parsing fails (yielding `NaN` or negative values) results in unhandled query exceptions and potential Denial of Service (DoS) 500 errors.
**Learning:** User inputs passed into ORM pagination methods must explicitly fall back from `NaN` and enforce boundaries, as `parseInt` alone does not guarantee a valid boundary or positive integer.
**Prevention:** Always validate and enforce valid numerical bounds using functions like `isNaN` and `Math.max` before passing parsed integers to database query parameters.
