...
## 2026-07-01 - Unhandled NaN input in Pagination Parameters
**Vulnerability:** The API endpoints used `parseInt` directly on URL query parameters like `offset` without validation or fallback from `NaN`. When a non-numeric string (like "invalid") was passed, `NaN` was sent to Prisma methods (`skip`, `take`), resulting in an unhandled query exception and a potential 500 server error, creating a minor DoS risk.
**Learning:** Functions like `parseInt` return `NaN` for invalid inputs. These values are not valid for database query operations and can cause unhandled exceptions if directly passed to ORMs like Prisma.
**Prevention:** Always explicitly validate parsed integer inputs, checking for `isNaN()`, and provide a safe fallback value (e.g., 0) before passing them to backend services or database queries.
