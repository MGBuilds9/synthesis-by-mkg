...
## 2026-02-13 - Missing Rate Limiting on Messages Endpoint
**Vulnerability:** The GET /api/messages/list endpoint was vulnerable to Denial of Service (DoS) due to missing rate limiting.
**Learning:** High-cost read operations (like database joins or complex queries) on sensitive endpoints can be targeted for resource exhaustion. All sensitive or resource-intensive endpoints must have rate limiting.
**Prevention:** Ensure rate limiting is consistently applied across all relevant API routes, especially those performing database queries.
