...
## 2026-06-14 - Bash Injection in GitHub Actions via PR Title and Commit Message
**Vulnerability:** The `github-to-linear-sync.yml` workflow directly interpolated `${{ github.event.head_commit.message }}` and `${{ github.event.pull_request.title }}` into bash scripts. This allows attackers to execute arbitrary shell commands if those fields contain malicious payload like backticks or \`$()\`.
**Learning:** Inline interpolation of any user-controlled GitHub context variable in bash scripts creates shell injection vulnerabilities.
**Prevention:** Always pass user-controlled input to bash scripts via the `env` context block in GitHub Actions and reference them as environment variables.
