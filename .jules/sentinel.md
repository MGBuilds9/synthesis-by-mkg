...
## 2026-08-05 - Secure GitHub Action Variables
**Vulnerability:** A GitHub Action workflow (`github-to-linear-sync.yml`) passed `github.event.pull_request.number` directly into a bash script using inline string interpolation.
**Learning:** Even GitHub Action context fields perceived as safe from shell injection (e.g., PR numbers) should be passed to bash scripts via the `env` block to establish secure patterns and pass SAST scanner checks.
**Prevention:** Always pass GitHub Action context variables into bash scripts through the `env` block.
