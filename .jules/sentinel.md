...
## 2026-06-24 - Unhandled NaN in Prisma Queries
**Vulnerability:** Parsing URL query parameters with `parseInt` without validating against `NaN` allows users to pass non-numeric inputs. When passed to Prisma methods like `skip` or `take`, this causes unhandled exceptions leading to a Denial of Service (500 errors).
**Learning:** All parsed numeric inputs must be explicitly checked for `NaN` before being used in database queries.
**Prevention:** Always validate numeric inputs and provide a safe fallback using `isNaN()` (e.g., `const offset = Math.max(0, isNaN(rawOffset) ? 0 : rawOffset);`).
