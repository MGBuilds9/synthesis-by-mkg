...
## 2026-05-07 - Insecure Logging in Sync Engine\n**Vulnerability:** The sync engine (`lib/sync/engine.ts`) used `console.error` to log errors when background syncing failed. Since the error object could contain internal application state or stack traces, this was an insecure logging vulnerability.\n**Learning:** Just like API routes, background tasks must also avoid raw `console.error` logs to prevent internal information from leaking to uncontrolled output streams.\n**Prevention:** Always use a centralized logger (like Winston via `@/lib/logger`) configured to properly sanitize and route log output, wrapping errors in structured properties (e.g., `logger.error(msg, { error })`).\n
