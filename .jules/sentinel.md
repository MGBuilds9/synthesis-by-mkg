...
## 2026-03-29 - Missing Rate Limiting on Sensitive Endpoint
**Vulnerability:** The `/api/messages/list` endpoint was missing rate limiting controls, exposing it to potential denial-of-service (DoS) attacks and data scraping, despite another similar endpoint (`/api/files/list`) having proper controls.
**Learning:** Security controls must be applied consistently across all endpoints. Finding a protection on one endpoint does not guarantee its presence on others.
**Prevention:** Always implement a central rate limiting mechanism and apply it universally to all authenticated or resource-intensive endpoints.
