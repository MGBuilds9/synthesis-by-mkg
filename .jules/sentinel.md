...
## 2024-03-20 - Unvalidated AI URLs leading to XSS
**Vulnerability:** The `MessageList` component rendered AI-provided URLs (`source.url`) directly into `href` attributes without validation, allowing a hallucinating AI or attacker-controlled context to inject `javascript:` URIs.
**Learning:** Never trust URLs, even those generated by LLMs or returned by internal services based on user context. AI outputs must be treated as untrusted user input and sanitized accordingly before rendering in the DOM.
**Prevention:** Always validate URLs against an explicit allowlist of safe protocols (e.g., `http://` and `https://`) before rendering them in `href` attributes.
