...
## 2026-09-19 - Prevent Invalid Pagination Input (Denial of Service)
...
## 2026-09-19 - Conditionally remove unsafe-eval from CSP in Next.js
**Vulnerability:** The Content-Security-Policy `script-src` directive statically included `'unsafe-eval'`, posing an XSS risk in production.
**Learning:** Next.js requires `'unsafe-eval'` for Fast Refresh during development, but it should be stripped out when deploying to production environments.
**Prevention:** Conditionally construct the CSP header based on `process.env.NODE_ENV === 'production'` rather than relying on a static string.
