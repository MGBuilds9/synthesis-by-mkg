...
## 2026-09-25 - Conditionally disable 'unsafe-eval' in Next.js CSP
**Vulnerability:** The Content-Security-Policy (CSP) hardcoded 'unsafe-eval' in the script-src directive for all environments.
**Learning:** Next.js development requires 'unsafe-eval' for Fast Refresh, but including it in production completely defeats the purpose of the CSP by allowing arbitrary code execution if an XSS vulnerability exists.
**Prevention:** Use `process.env.NODE_ENV !== 'production'` to conditionally inject 'unsafe-eval' into the CSP only during local development.
