...
## 2024-05-18 - NaN Input Causing DoS via Prisma
**Vulnerability:** The API endpoints `/api/messages/list` and `/api/files/list` passed parsed `offset` search parameter directly into Prisma's `skip` parameter without validating `isNaN()`. If `?offset=invalid` was sent, `parseInt` returned `NaN`, which caused unhandled query exceptions in Prisma, potentially leading to a Denial of Service (500 errors).
**Learning:** `parseInt` on invalid strings returns `NaN`, and Prisma fails abruptly when `NaN` is given for numeric fields like `skip` or `take`. Standard type safety doesn't protect against `NaN` as it's a valid `number` type in TypeScript.
**Prevention:** Always validate `NaN` from parsed URL query parameters using `isNaN(num)` before passing them into the ORM or DB engine. Add safe fallback logic like `Math.max(0, isNaN(rawOffset) ? 0 : rawOffset)`.
## 2024-05-18 - Command Injection in GitHub Actions via head_commit.message
**Vulnerability:** A GitHub Action workflow (`github-to-linear-sync.yml`) passed user-controlled input (`${{ github.event.head_commit.message }}`) directly into a bash script using inline string interpolation. If the commit message contained backticks or subshells (e.g., in a markdown snippet like \`/api/messages/list\`), the shell attempted to evaluate them as commands, leading to command injection and workflow failure (exit code 126).
**Learning:** Inline string interpolation of GitHub context variables (especially those containing user input like `head_commit.message` or `pull_request.body`) into bash scripts creates critical shell injection vulnerabilities.
**Prevention:** Always pass user-controlled input to bash scripts via the `env` context block in GitHub Actions (e.g., `HEAD_COMMIT_MESSAGE: ${{ github.event.head_commit.message }}`) and reference them as environment variables (e.g., `$HEAD_COMMIT_MESSAGE`), rather than interpolating them directly into the script content.
