...
## 2026-09-24 - Remove unsafe-eval from production CSP
**Vulnerability:** The `script-src` directive in the Content-Security-Policy header included `'unsafe-eval'` in production, which increases the risk of Cross-Site Scripting (XSS) by allowing string-to-code execution.
**Learning:** Next.js development server requires `'unsafe-eval'` for Fast Refresh to function, leading to it accidentally being hardcoded into the global middleware for all environments.
**Prevention:** Conditionally construct CSP headers based on `process.env.NODE_ENV` to ensure relaxed development policies are stripped before production deployment.
