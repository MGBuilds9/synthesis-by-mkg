...
## 2026-09-20 - Conditionally removed 'unsafe-eval' from Next.js CSP
**Vulnerability:** The Content-Security-Policy globally permitted 'unsafe-eval' in the script-src directive in both development and production.
**Learning:** Next.js requires 'unsafe-eval' for Hot Module Replacement (HMR) and Fast Refresh during local development, leading developers to hardcode it into the CSP, which unintentionally opens up XSS vectors in production.
**Prevention:** Always construct CSP directives dynamically based on process.env.NODE_ENV, appending 'unsafe-eval' exclusively when not in 'production'.
