...
## 2026-08-31 - Remove unsafe-eval from production CSP
**Vulnerability:** The Content-Security-Policy (CSP) script-src directive unconditionally included 'unsafe-eval', which significantly increases the risk of Cross-Site Scripting (XSS) attacks in production.
**Learning:** Next.js Fast Refresh relies on 'unsafe-eval' during development, meaning it cannot simply be removed. It must be conditionally stripped out in production environments.
**Prevention:** Always evaluate environment contexts (e.g., process.env.NODE_ENV) when defining CSP rules to ensure development necessities do not leak into production security policies.
