...
## 2026-09-18 - Remove unsafe-eval from production CSP
**Vulnerability:** The Content-Security-Policy (CSP) globally included `unsafe-eval` in the `script-src` directive.
**Learning:** Next.js requires `unsafe-eval` during development for Fast Refresh, but including it in production exposes the application to Cross-Site Scripting (XSS) via string-to-code execution.
