...
## 2026-09-01 - Remove unsafe-eval from CSP in production
**Vulnerability:** Next.js development requires `'unsafe-eval'` in Content-Security-Policy for Fast Refresh, which often accidentally leaks into production environments, increasing the risk of XSS attacks.
**Learning:** CSP policies in Next.js middleware should be environment-aware. We must dynamically construct the policy based on `process.env.NODE_ENV`.
**Prevention:** Always conditionally strip `'unsafe-eval'` in the `script-src` directive when the environment is `production`.
