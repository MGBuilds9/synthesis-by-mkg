...
## 2026-06-20 - Denial of Service via Prisma NaN Propagation
**Vulnerability:** The `/api/messages/list` and `/api/files/list` endpoints passed parsed `offset` and `limit` values directly to Prisma without checking for `NaN`. When a client sends a non-numeric string like `?offset=abc`, `parseInt` returns `NaN`, which causes Prisma's query engine to throw an unhandled exception, resulting in a 500 error and potential Denial of Service.
**Learning:** Validating inputs from URLs using `parseInt` is insufficient because it can evaluate to `NaN`, and most ORMs (like Prisma) cannot serialize or handle `NaN` safely in query methods like `skip` or `take`.
**Prevention:** Always explicitly validate that parsed numeric inputs from untrusted sources are not `NaN` (e.g. using `isNaN()`) and provide a safe fallback value before passing them to ORMs or database drivers.
