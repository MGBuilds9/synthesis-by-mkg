...
## 2026-06-29 - NaN in Prisma skip
**Vulnerability:** Unsanitized pagination offset parsed via parseInt can result in NaN being passed to Prisma's skip parameter.
**Learning:** Prisma handles NaN by throwing an unhandled error which can cause DoS or 500s. parseInt does not fall back to integers for non-numeric strings.
**Prevention:** Always validate numeric inputs using isNaN or enforce constraints using Math.max/min before passing them to DB queries.
