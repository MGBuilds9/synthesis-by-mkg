...
## 2026-09-04 - Fix XSS Vulnerability in AI MessageList
**Vulnerability:** The AI Assistant could potentially generate untrusted source URLs with malicious protocols like `javascript:`, leading to Cross-Site Scripting (XSS) when rendered in `href` attributes.
**Learning:** URLs generated by Large Language Models or provided by internal context retrieval services should always be treated as untrusted user input, as hallucinations or poisoned context can inject malicious payloads.
**Prevention:** Always parse and explicitly validate the `protocol` of URLs (allowing only `http:` and `https:`) before using them in dynamic DOM elements like anchor tag `href` properties.
