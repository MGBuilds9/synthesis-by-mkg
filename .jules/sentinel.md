...
## 2026-06-25 - Prisma Unhandled Exception via NaN Parameter
**Vulnerability:** The API endpoints (`/api/messages/list` and `/api/files/list`) passed parsed URL query parameters (`parseInt(searchParams.get('offset'))`) directly to Prisma's `skip` method without checking for `NaN`. If a user provides an invalid non-numeric offset string, `parseInt` returns `NaN`, causing Prisma to throw an unhandled exception and resulting in a Denial of Service (500 Error).
**Learning:** Functions like `parseInt` can return `NaN`, which many ORMs and libraries do not handle gracefully. Unvalidated numeric conversions can lead to unhandled runtime exceptions.
**Prevention:** Always explicitly validate parsed numeric inputs (e.g., using `isNaN()`) and provide safe fallback values before passing them to database queries or critical operations.
