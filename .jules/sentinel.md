...
## $(date +%Y-%m-%d) - Plaintext Password Exposure in Client Logs
**Vulnerability:** The sign-in page (`app/auth/signin/page.tsx`) logged the user's plaintext password to the browser console during the sign-in process.
**Learning:** Development console logs are often left in production or can be intercepted if sensitive data is included.
**Prevention:** Never log passwords, tokens, or other sensitive credentials in client-side or server-side logs.
