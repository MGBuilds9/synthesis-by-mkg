...
## 2026-09-18 - Development Features in Production CSP
**Vulnerability:** The `Content-Security-Policy` headers allowed `'unsafe-eval'` globally across all environments.
**Learning:** Next.js Fast Refresh requires `'unsafe-eval'` in development for hot module reloading, but deploying this to production creates a significant XSS risk, allowing injected scripts to execute string payloads.
**Prevention:** Always conditionally configure environment-specific CSP directives, explicitly stripping development conveniences like `'unsafe-eval'` when `process.env.NODE_ENV === 'production'`.
