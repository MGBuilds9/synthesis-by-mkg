...
## 2026-06-18 - Unhandled NaN Input to Prisma Methods
**Vulnerability:** Calling Prisma query methods (like `skip` or `take`) with `NaN` resulted from `parseInt` without validation causes unhandled exceptions leading to potential Denial of Service (DoS).
**Learning:** Always validate user inputs parsed via `parseInt` before passing them to the database ORM to prevent crashes.
**Prevention:** Use `isNaN()` to check parsed numerics and provide a safe fallback (e.g., `const limit = Math.max(1, isNaN(rawLimit) ? 50 : rawLimit)`).
