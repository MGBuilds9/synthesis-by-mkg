...
## 2026-06-23 - Unhandled NaN in Prisma Queries
**Vulnerability:** Unvalidated numeric URL parameters (`offset`) passed to `parseInt` can result in `NaN`, which causes unhandled exceptions (500 errors) when passed to Prisma's `skip` or `take` methods.
**Learning:** Prisma does not safely fallback when receiving `NaN` for pagination arguments, which can be leveraged for application-level DoS or log bloat.
**Prevention:** Always validate parsed numeric inputs using `isNaN()` and enforce boundaries (e.g., `Math.max(0, ...)`) before passing them to Prisma.
