...
## 2026-09-07 - Prevent Invalid Pagination Input (Denial of Service)
...
## 2026-09-07 - Conditional CSP Directives
**Vulnerability:** The application broadly permitted `'unsafe-eval'` in its Content-Security-Policy `script-src` directive, weakening XSS protections across all environments.
**Learning:** Next.js development mode requires `'unsafe-eval'` for Fast Refresh (Webpack HMR) and debugging, but it is unnecessary and dangerous in production.
**Prevention:** Always apply `'unsafe-eval'` conditionally in Next.js applications (e.g., checking `process.env.NODE_ENV !== 'production'`), ensuring production environments maintain strict CSP protections.
