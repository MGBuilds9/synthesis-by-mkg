...
// Bolt: Extracted domain scope types into module-level Sets for O(1) lookups
// and to prevent redundant array allocations during every request execution
const EMAIL_SCOPES = new Set(['GMAIL_LABEL', 'OUTLOOK_FOLDER'])
const CHAT_SCOPES = new Set([
  'DISCORD_SERVER', 'DISCORD_CHANNEL', 'WHATSAPP_ACCOUNT',
  'SLACK_WORKSPACE', 'SLACK_CHANNEL', 'TELEGRAM_CHAT',
  'TEAMS_WORKSPACE', 'TEAMS_CHANNEL'
])
    ...
    if (useContext) {
      ...
      if (contextDomains) {
        activeScopes = activeScopes.filter((scope: any) => {
          ...
          // Bolt: Optimized to use O(1) Set lookups instead of allocating new arrays
