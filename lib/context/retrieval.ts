...
// Bolt: Module-level Set constants for O(1) membership checks to eliminate redundant
// object allocations and garbage collection overhead in the request path
const MESSAGE_SCOPES = new Set(['DISCORD_CHANNEL', 'GMAIL_LABEL', 'OUTLOOK_FOLDER'])
...
export async function retrieveAIContext(options: ContextOptions, preFetchedScopes?: any[]) {
  ...
  contextScopes.forEach((contextScope: any) => {
    ...
    if (MESSAGE_SCOPES.has(scopeType)) {
